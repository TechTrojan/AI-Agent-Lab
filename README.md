...
# if rollup/esbuild native module errors persist:
npm run repair:windows
npm run build
```


If you get `npm error Missing script: "repair:windows"`, your local clone is behind. Run:
...
Then verify scripts exist:

```powershell
npm run | Select-String "repair"
```
